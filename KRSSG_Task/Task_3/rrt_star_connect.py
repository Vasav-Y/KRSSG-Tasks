...
def lowestCostNode(nodeTree, xProbableNode, yProbableNode, costNode):
    ...
    # finding valid points to check in 20*20 (in sqaure region)
    for i in range(xProbableNode-20, xProbableNode+20):
        if i >= 0 and i < size[1]:
            xvals.append(i)
    for i in range(yProbableNode-20, yProbableNode+20):
        if i >= 0 and i < size[0]:
            yvals.append(i)

    #finding valid points in a circular region of radius 20 takes more time
    # for y in range (yProbableNode-20, yProbableNode+20):
#         for x in range (xProbableNode-20, xProbableNode+20):
#             xterm = ( x - xProbableNode)
#             yterm = ( y - yProbableNode)
# 
#             if((400 - (xterm**2 + yterm**2))> 0):
#                 xvals.append(x)
#                 yvals.append(y)
...
def changeColour(image, node1, node2):
    ...
    for i in range(0, xDisplacement, step):
        ...
        image.itemset((k, node2[1]+i, 2), 10)
    ...
    for i in range(0, yDisplacement, step):
        ...
        image.itemset((node2[0]+i, j, 2), 10)
...
def createNode(img, nodeTree, connectionTree, parentNode, costNode, listNode,count):
    ...
    while not d:
        ...
        if ClosestNode[2] != 0:
            xProbableNode = ClosestNode[1] + \
                int((xRad - ClosestNode[1])*l/ClosestNode[2])
            yProbableNode = ClosestNode[0] + \
                int((yRad - ClosestNode[0])*l/ClosestNode[2])
        d = ClosestNode[2]
...
def showTree(img, parentNewNode, parentNode, costNewNode,  costNode, availableConnections, startNewPoint, startPoint):
    ...
    for connection in availableConnections:
        pathLength = costNewNode[connection[0][0]][connection[0][1]] + \
            costNode[connection[1][0]][connection[1][1]] + connection[2]
